...
const reviewed = await pipeline(
  ...
  (d) =>
    agent(
      `이 레포(React19+TS+Vite+Tailwind, FSD)의 변경사항을 리뷰한다. 기준은 AGENTS.md와 정본 규칙 .claude/rules.md다.\n` +
        ...
        (f) => () =>
          agent(
            `다음 리뷰 발견이 실제 문제인지 적대적으로 검증하라. 기본값은 "반증"이며, 근거가 확실할 때만 isReal=true.\n` +
              `파일: ${f.file}:${f.line}\n요약: ${f.summary}\n제안: ${f.suggestion}\n` +
              `해당 파일을 직접 읽고 .claude/rules.md 기준으로 판단하라.`,
...
const report = await agent(
  `아래는 관점별 리뷰 후 적대적 검증을 통과한 발견 목록(JSON)이다. 이 레포 규칙(AGENTS.md · .claude/rules.md) 기준으로 한국어 리뷰 리포트를 작성하라.\n` +
