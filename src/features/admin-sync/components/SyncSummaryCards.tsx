...
] satisfies { key: keyof SyncSummary; label: string; valueClass: string; note: string; noteClass: string }[];
...
export function SyncSummaryCards() {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {CARD_METAS.map((card) => (
        <div key={card.key} className="rounded-2xl border border-slate-100 bg-white p-4.5 text-left shadow-sm">
