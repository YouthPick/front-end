...
import { fetchUserProfile, profileResponseToUserProfile, saveUserProfile } from "../api/profile-api";
...
export function useProfilePersistence(): UseProfilePersistenceResult {
  ...
  const loadProfile = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "프로필 조회 API 호출에 실패했습니다.";
  ...
  const saveProfile = useCallback(async (profile: UserProfile) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "프로필 저장 API 호출에 실패했습니다.";
      setErrorMessage(message);
      throw error;
    } finally {
      setIsSaving(false);
