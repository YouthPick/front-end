...
<div className="space-y-6 animate-in fade-in duration-300 max-w-3xl mx-auto">
  ...
  <div className="rounded-3xl bg-white border border-slate-100 p-6 text-left space-y-4 shadow-sm">
    <h4 className="text-xs font-extrabold text-slate-800 border-b border-slate-100 pb-3">보관중인 관심 정책 목록 ({savedPolicies.length}건)</h4>
    <div className="flex flex-wrap gap-1.5 text-[10px] font-bold">
      {isFavoriteLoading && (
        <span className="rounded-full bg-slate-100 px-2 py-0.5 text-slate-500">관심 API 동기화 중</span>
      )}
      {favoriteErrorMessage && (
