      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-slate-100 bg-white p-4.5 text-left shadow-sm">
          ...
          <span className="text-[9px] text-emerald-500 font-bold mt-1 block">
            ✓ 공개 {adminMetrics.policy.exposedCount.toLocaleString()}건 · 활성{" "}
            {adminMetrics.policy.activeCount.toLocaleString()}건
          </span>
        </div>
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-4.5 text-left shadow-sm">
          ...
          <span className="text-xl font-black text-primary block mt-1">
            {(adminMetrics.user.favoriteCount + adminMetrics.user.readStateCount).toLocaleString()}
            건
          </span>
          <span className="text-[9px] text-slate-400 font-bold mt-1 block">
            프로필 {adminMetrics.user.profileCount.toLocaleString()} · 관심{" "}
            {adminMetrics.user.favoriteCount.toLocaleString()} · 읽음{" "}
            {adminMetrics.user.readStateCount.toLocaleString()}
          </span>
        </div>
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-4.5 text-left shadow-sm">
          ...
          <span className="text-xl font-black text-slate-800 block mt-1">
            {adminMetrics.sync.failedJobCount.toLocaleString()}건 /{" "}
            {adminMetrics.sync.runningJobCount.toLocaleString()}건
          </span>
          <span className="text-[9px] text-slate-400 font-bold mt-1 block">
            최근 상태 {adminMetrics.sync.lastJobStatus ?? "이력 없음"} · 오류{" "}
            {adminMetrics.sync.lastJobErrorCount.toLocaleString()}건
          </span>
        </div>
      </div>
