...
const ProfileSetupPage = lazy(() =>
  import("@pages/profile").then((module) => ({ default: module.ProfileSetupPage })),
);
const RecommendPage = lazy(() =>
  import("@pages/recommend").then((module) => ({ default: module.RecommendPage })),
);
const TrackerPage = lazy(() =>
  import("@pages/tracker").then((module) => ({ default: module.TrackerPage })),
);
...
const KNOWN_POLICY_CATEGORIES: Policy["category"][] = [
  "일자리",
  "주거",
  "교육",
  "복지문화",
...
function favoritePolicyToPolicy(item: FavoritePolicyDto): Policy {
  ...
  return {
    ...
    description:
      "관심 정책 API에서 불러온 저장 항목입니다. 상세 내용을 눌러 최신 공고 정보를 확인하세요.",
    target: "전체",
    deadline: "상세확인",
    logoType:
      category === "일자리"
...
export default function App() {
    ...
    fetchCurrentUser(controller.signal)
      .then(async (currentUser) => {
        ...
        if (requiresProfileSetup(currentUser) && activeView !== "profile_setup") {
          ...
          showToast(
            "프로필 설정이 필요합니다. 맞춤 추천을 위해 온보딩을 먼저 진행해 주세요.",
            "info",
          );
          return;
        }
  ...
  useEffect(() => {
    ...
    const returnView =
      (window.sessionStorage.getItem(OAUTH_RETURN_VIEW_STORAGE_KEY) as AppView | null) ??
      previousView;
    ...
    completeOAuthLogin(pendingProvider, code)
      ...
      .catch(() => {
        ...
        setCanAccessAdmin(false);
        showToast(
          "OAuth 로그인 처리에 실패했습니다. 백엔드 OAuth 설정을 확인해 주세요.",
          "warning",
        );
        navigateToPath("login");
      })
  ...
  const handleSocialLogin = async (provider: string) => {
    try {
      ...
      showToast(
        `${provider} OAuth 로그인 URL 생성에 실패했습니다. 백엔드 OAuth 환경변수를 확인해 주세요.`,
        "warning",
      );
    }
  };
  ...
  const savedPolicies = useMemo(() => {
    ...
    return savedPolicyIds
      .map(
        (id) =>
          apiFavoritePolicies.find((policy) => policy.id === id) ??
  ...
  const readTrackedPolicyIds = useMemo(
    () =>
      Array.from(
        new Set([
  ...
  const runAdminSync = async () => {
    try {
      ...
      showToast(
        error instanceof Error ? error.message : "정책 동기화 실행에 실패했습니다.",
        "warning",
      );
    }
  };
  ...
  const runSearchIndexRebuild = async () => {
    try {
      ...
      showToast(
        error instanceof Error ? error.message : "검색 인덱스 재생성에 실패했습니다.",
        "warning",
      );
    }
  };
        ...
        >
          ...
          {activeView === "search" && (
            <SearchPage
              ...
              onSearchQueryChange={setSearchQuery}
              onSearchSubmit={() =>
                showToast(`검색 쿼리가 적용되었습니다: '${searchQuery}'`, "info")
              }
